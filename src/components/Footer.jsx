  ...
  return (
    <section className={`${style.flexCenter} ${style.paddingY} flex-col`}>
      ...
      <div className="w-full flex justify-between items-center md:flex-row flex-col pt-6 border-t-[1px] border-t-[#3f3e45]">
        <p className="font-poppins font-normal text-center text-[18px] leading-[27px] text-white">2023 HooBank. Made with love by rizkiromadoni</p>
        <div className="flex flex-row md:mt-0 mt-6">
          {socialMedia.map((social, index) => (
            <img id={social.id} src={social.icon} alt={social.id} className={`w-[21x] h-[21px] object-contain cursor-pointer ${index !== socialMedia.length - 1 ? 'mr-6' : 'mr-0'}`}/>
          ))}
        </div>
      </div>
    </section>
